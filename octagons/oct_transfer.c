...
static inline void hmat_forget_var(dbm* m, size_t dim, size_t d)
{
  ...
  size_t k;
  for (k=0;k<2*d;k++) {
    /* bound_set_infty(*getdbm(m,matpos(2*d,k)),1); */
    /* bound_set_infty(*getdbm(m,matpos(2*d+1,k)),1); */
    setdbminfty(m,matpos(2*d,k));
    setdbminfty(m,matpos(2*d+1,k));
  }
  for (k=2*d+2;k<2*dim;k++) {
    /* bound_set_infty(*getdbm(m,matpos(k,2*d)),1); */
    /* bound_set_infty(*getdbm(m,matpos(k,2*d+1)),1); */
			...
			size_t d, bool* respect_closure)
{
  ...
  if (u.type==ZERO ) {
    ...
    *respect_closure = false; /* does not respect closure */
    ...
    if (!cinf) {
      /* no bound is infinite */
      //bound_set(*getdbm(m,matpos(2*d,2*d+1)),cb); /* bound for -x */
